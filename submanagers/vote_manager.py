...
class VoteManager(Manager):
    ...
    def __process(self, interval: int):
        ...
        if response and len(response):
            ...
            for entry in response:
                ...
                elif entry.type == entry.EntryType.CHAT and entry.message.startswith("Is there an alpha "):
                    vote_t = entry.message.split("Is there an alpha ")[1].split("?")[0]
    ...
    def __handle_alpha_vote_success(self):
        ...
        self.ftp.connect()
        save_path = self.ftp.download_save_file("Aberration_WP.ark", Path.cwd())
        self.ftp.close()
    ...
    def handle_vote_result(self):
        ...
        if self.vote_type == "Test":
            self.rcon.send_message(f"Test vote ended. Yes: {yes_votes}, No: {no_votes}, {tribes} tribes had yes vote")
        elif self.vote_type == "reaper" or self.vote_type == "basilisk" or self.vote_type == "karkinos":
            if tribes == self.__get_nr_of_tribes():
