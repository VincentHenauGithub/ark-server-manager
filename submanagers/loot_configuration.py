...
def _get_loot_distribution(turrets: int, mixed: bool) -> dict:
    """
    Returns the loot distribution based on the number of turrets.
    """
    for distribution in LOOT_DISTRIBUTIONS:
        if (distribution["min_turrets"] <= turrets <= distribution["max_turrets"]) and (distribution["mixed"] == mixed):
            return distribution
    return None

def _get_random_category(no_resources: bool) -> str:
    """
    Returns a random loot category.
    """
    categories = ["saddles", "weapons", "resources", "armor"]
    if no_resources:
        categories.remove("resources")
...
def add_loot(manager, turrets: int, save: AsaSave, container: StructureWithInventory | TamedDino, equipment_api: EquipmentApi, mixed: bool = False):
    """
    Generates loot based on the number of turrets.
    """
    distribution = _get_loot_distribution(turrets, mixed)
    if not distribution:
        raise ValueError(f"No loot distribution found for {turrets} turrets")
    
    amount = 6 if not isinstance(container, TamedDino) else 3
    no_resources = False if not isinstance(container, TamedDino) else True

    loot: List[Resource | Armor | Weapon | Saddle | Shield] = []
    for _ in range(amount):
        category = _get_random_category(no_resources)
        if category == "saddles":
            s: Saddle = _get_saddle_loot(distribution, equipment_api)
            loot.append(s)
            if manager:
                manager._print(f"Added saddle: {s.get_short_name()} with armor {s.armor:.2f} and durability {s.durability:.2f}")
        elif category == "weapons":
            w: Weapon = _get_weapon_loot(distribution, equipment_api)
            loot.append(w)
            if manager:
                manager._print(f"Added weapon: {w.get_short_name()} with damage {w.damage:.2f} and durability {w.durability:.2f}")
        elif category == "resources":
            loot.extend(_get_resource_loot(distribution, save, container.uuid))
        elif category == "armor":
            a_o_s: Armor | Shield = _get_armor_loot(distribution, equipment_api)
            if isinstance(a_o_s, Shield):
                s: Shield = a_o_s
                if manager:
                  manager._print(f"Added shield: {s.get_short_name()} with durability {s.durability:.2f}")
            else:
                a: Armor = a_o_s
                if manager:
                    manager._print(f"Added armor: {a.get_short_name()} with armor {a.armor:.2f} and durability {a.durability:.2f}")

            loot.append(a_o_s)

    original_items = list(container.inventory.items.keys())

    for item in loot:
        container.add_item(item.uuid)
        item.update_binary()

    for item in original_items:
        container.remove_item(item)
        
    container.update_binary()
